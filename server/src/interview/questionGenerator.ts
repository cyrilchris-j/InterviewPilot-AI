...
export class QuestionGenerator {
  generate(planItem: PlanItem, context: QuestionContext): InterviewQuestion {
  ...
  followUp(planItem: PlanItem, context: QuestionContext): InterviewQuestion {
    const build = this.buildContext(planItem, context);
    const text =
      build.previousEvaluation && build.previousAnswer
  ...
  private buildConcept(build: BuildContext): string {
    ...
    if (build.difficulty === "easy") {
      return (
        `Walk me through ${focus} on Day ${build.day.day} (${build.day.title}) the way you would for a senior engineer joining the team` +
        `${scaffold} What would you want their mental model to capture — not a definition, the behavior.`
      );
    }
    if (build.difficulty === "medium") {
      return (
        `We lean on ${this.tools(build)} every day in the chatbot. Walk me through ${focus} in our healthcare context and then name the first thing that would look wrong in production` +
        `${scaffold} — where do those signals actually surface?`
      );
    }
    return (
      `Explain ${focus} as if you were defending it in a design review, then tell me the two quietest ways it degrades in production` +
      `${deepen} How would you notice them before a user does?`
    );
  }

  private buildScenario(build: BuildContext): string {
    const focus = this.focusPhrase(build.objective);
    ...
    if (build.difficulty === "easy") {
      return (
        `A teammate on the healthcare chatbot asks for a coverage question that hinges on ${focus} from Day ${build.day.day} (${build.day.title}).` +
        ` Walk me through the request from the API boundary to the answer — where does the real work happen?${reiterateStruggling}`
      );
    }
    if (build.difficulty === "medium") {
      return (
        `A user asks a question that spans both a structured plan lookup and the messy edge cases of ${focus}. ` +
        `Trace the request end to end and explain when you would merge results versus trust one source.`
      );
    }
    return (
      `Leadership wants the chatbot to answer a fully novel, multi-source healthcare question grounded in ${focus}. ` +
      `Walk through how you compose the answer, and where you stop trusting each source when the answers conflict.`
    );
  }

  private buildArchitecture(build: BuildContext): string {
    const focus = this.focusPhrase(build.objective);
    if (build.difficulty === "easy") {
      return (
        `Sketch the components you would wire together to make ${focus} work on Day ${build.day.day} (${build.day.title}). ` +
        `Where does each piece live, and what do you connect to what?`
      );
    }
    if (build.difficulty === "medium") {
      return (
        `Design the backend for ${focus}: where do ${this.tools(build)} sit, how does state flow, ` +
        `and what do you add so a second healthcare domain can hook in without re-wiring the core?`
      );
    }
    return (
      `You are designing the production architecture for a chatbot whose core capability is ${focus}. ` +
  ...
  private buildDebugging(build: BuildContext): string {
    const focus = this.focusPhrase(build.objective);
    if (build.difficulty === "easy") {
      return (
        `One user complaint quietly becomes a trend: answers tied to ${focus} are confidently wrong. ` +
        `Give me your first three concrete steps to isolate whether this is the data, the retrieval, or the prompt.`
      );
    }
    if (build.difficulty === "medium") {
      return (
        `You shipped a change that touches ${focus} today and retrieval quality reports dropped 40%. ` +
        `How do you bisect that to a single root cause without shipping more changes, and what metric proves you fixed it?`
      );
    }
    return (
      `Users report that answers tied to ${focus} are spotless for English queries but subtly wrong for the same question in another language. ` +
      `What's your thesis, how do you prove it, and what production evidence do you pull to confirm before you touch anything?`
    );
  }

  private buildProduction(build: BuildContext): string {
    const focus = this.focusPhrase(build.objective);
    if (build.difficulty === "easy") {
      return (
        `You're on call for ${focus} on Day ${build.day.day} (${build.day.title}) in production. ` +
        `What does "healthy" look like, and who or what notices if it silently regresses?`
      );
    }
    if (build.difficulty === "medium") {
      return (
        `You're shipping a production launch that relies on ${focus}. Walk me through the launch checklist — ` +
    ...
    return (
      `An incident pager wakes you right in the middle of ${focus}: the team realizes ${this.tools(build)} "looks healthy" while users still get wrong answers. ` +
      `Design the production telemetry and the runbook that would have caught it, and tell me the decision that rolls it back.`
    );
  }

  private buildTradeoff(build: BuildContext): string {
    const [optionA, optionB] = this.twoTools(build);
    const focus = this.focusPhrase(build.objective);
    if (build.difficulty === "easy") {
      return (
        `We could approach ${focus} two different ways${this.optionPair(optionA, optionB)}. ` +
    ...
    if (build.difficulty === "medium") {
      return (
        `Pick between ${optionA ?? "a hosted solution"} and ${optionB ?? "building it in-house"} for ${focus} on ${build.day.title}. ` +
    ...
    return (
      `We have budget for exactly one strategy to carry ${focus} to production-grade. Compare ${optionA ?? "a managed option"} against ${optionB ?? "an open-ended internal build"}: ` +
  ...
  private buildFailureAnalysis(build: BuildContext): string {
    const focus = this.focusPhrase(build.objective);
  ...
  private buildAdaptiveFollowUp(build: BuildContext): string {
    ...
    return (
      `You said "${quote}".${verdictHook} ${evaluation.followUpHint}` +
  ...
  private buildReflectiveFollowUp(build: BuildContext): string {
  ...
  private dedup(planItem: PlanItem, text: string, askedKeys: Set<string>, tag?: string): InterviewQuestion {
  ...
  private buildContext(planItem: PlanItem, context: QuestionContext): BuildContext {
    return {
      day: planItem.day,
      objective: planItem.objective,
      difficulty: planItem.difficulty,
  ...
  private tools(build: BuildContext): string {
    return build.day.tools.length ? build.day.tools.slice(0, 3).join(", ") : "the pipeline";
  }

  private twoTools(build: BuildContext): [string | undefined, string | undefined] {
  ...
  private optionPair(optionA: string | undefined, optionB: string | undefined): string {
    if (optionA && optionB) return ` — ${optionA} or ${optionB}`;
    return optionA ? ` — ${optionA} versus a hand-rolled fallback` : " — a hosted service versus a hand-rolled fallback";
  }

  private focusPhrase(objective: string): string {
  ...
  private gerundInterior(word: string): string {
  ...
  private static readonly SECONDARY_VERBS = new Set([
    "activate",
    "clean",
    "commit",
    "debug",
  ...
  private static readonly GERUNDS: Record<string, string> = {
    install: "installing",
  ...
  private quote(answer: string): string {
