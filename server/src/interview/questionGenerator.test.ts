...
import type { CandidateAnalysis, PlanItem, QuestionType } from "../types/domain.js";
...
function planItem(questionType: QuestionType, day = 10, difficulty: PlanItem["difficulty"] = "medium"): PlanItem {
...
function ask(questionType: QuestionType, overrides: Partial<Parameters<QuestionGenerator["generate"]>[1]> = {}): string {
...
test("generates each required question type with senior-interviewer phrasing", () => {
  const types: QuestionType[] = ["Scenario", "Architecture", "Debugging", "Production", "Follow-up"];
...
test("grounds questions in the curriculum day and the healthcare chatbot context", () => {
  ...
  assert.ok(
    text.includes("Day 10") || text.toLowerCase().includes("retrieval") || text.toLowerCase().includes("chatbot"),
    `expected curriculum-grounded scenario, got: ${text}`
  );
});

test("scaffolds weak-area candidates instead of demanding depth", () => {
  const weakAnalysis: CandidateAnalysis = {
    ...analysis,
    weakDays: [10]
  };
...
test("shapes objectives into gerund phrases so questions read naturally", () => {
  const createItem = (objective: string, day = 10) => ({ ...planItem("Concept", day), objective });
