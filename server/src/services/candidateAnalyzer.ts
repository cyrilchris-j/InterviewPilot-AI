import type { Candidate, CandidateAnalysis, CandidateProfile, Curriculum } from "../types/domain.js";
...
export class CandidateAnalyzer {
  private readonly profileBuilder: CandidateProfileBuilder;
  private readonly scoring: CandidateScoring;
  ...
  private describeStrengths(profile: CandidateProfile): string[] {
  ...
  private describeWeaknesses(profile: CandidateProfile): string[] {
    ...
    return profile.weakAreas.slice(0, 4).map((area) => {
      if (area.reason === "failed") return `${area.title} was attempted but not completed.`;
      if (area.reason === "struggled") return `${area.title} needed ${area.attempts} attempts.`;
      return `${area.title} was skipped.`;
    });
  }

  private riskNotes(profile: CandidateProfile, weakDays: number[]): string[] {
    const notes: string[] = [];
