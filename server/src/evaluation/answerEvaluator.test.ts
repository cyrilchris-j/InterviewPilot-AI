...
import type { InterviewQuestion } from "../types/domain.js";
...
test("returns the five required evaluation dimensions as integers 1-5", () => {
  const result = evaluator.evaluate(
    question,
    "First I would chunk the plans and claims, then generate embeddings for every chunk, monitor retrieval quality with an eval set, and roll back if latency regresses.",
    analysis
  );
...
test("strong answer escalates in the next action", () => {
  const stellar =
    "I would load the knowledge base embeddings into the vector database via a pipeline, because a batch " +
