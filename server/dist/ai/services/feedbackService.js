...
export class FeedbackService {
    ...
    formatTranscript(turns) {
        return turns
            .map((turn, index) => `Turn ${index + 1} (Day ${turn.question.day}: ${turn.question.dayTitle})\n` +
    ...
    formatTurns(turns) {
        return turns
            .map((turn) => `- Day ${turn.question.day} (${turn.question.dayTitle}): ${turn.evaluation.score}/5 (${turn.evaluation.verdict})`)
            .join("\n");
    }
}
