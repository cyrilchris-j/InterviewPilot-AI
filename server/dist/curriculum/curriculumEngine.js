...
export class CurriculumEngine {
    ...
    dayMap;
    moduleForDay;
    constructor(repository = new CurriculumRepository()) {
        ...
        this.moduleForDay = new Map(curriculum.modules.flatMap((module) => this.expandDays(module.days).map((dayNumber) => [dayNumber, module])));
    }
    getDay(dayNumber) {
    ...
    getQuestionPool(options = {}) {
        ...
        for (const day of this.repository.getDays()) {
            if (dayFilter && !dayFilter.has(day.day))
                continue;
            const module = this.getModuleForDay(day.day);
            if (moduleNumber !== undefined && module?.n !== moduleNumber)
                continue;
    ...
    allTopics(candidate) {
        ...
        const failed = new Set(candidate.missions.filter((mission) => !mission.passed && !mission.skipped).map((mission) => mission.day));
    ...
    topicOrder(topicA, topicB, prioritized) {
        ...
        if (topicA.moduleNumber !== topicB.moduleNumber)
            return topicA.moduleNumber - topicB.moduleNumber;
        return topicA.day - topicB.day;
    }
    rankOf(topic, prioritized) {
        if (prioritized.has(topic.day))
            return -1;
        return STATUS_RANK[topic.status];
    }
    statusOf(dayNumber, completed, failed, skipped) {
        if (completed.has(dayNumber))
            return "completed";
        if (failed.has(dayNumber))
            return "failed";
        if (skipped.has(dayNumber))
            return "skipped";
        return "pending";
    }
    expandDays(dayNumbers) {
